...
void gen_intermediate_code(AST_NODE *root)
{
	...
	if(root -> nodeType == FUNCTION_HDR){
		...
		temp_symtbl->func_off = triple_list_index;
...
int if_code(AST_NODE *p)
{
	...
	if(childNum == 5)//if without else
	{
		 ...
		 */
		switch(exp){
			case -1:add_triple(if_not_op, (int)temp_ID, triple_list_index + 2, -1, 0, 1);break;
			case -2:add_triple(if_not_op, const_value, triple_list_index + 2, -1, 2, 1);break;
			case -3:add_triple(if_not_op, const_value, triple_list_index + 2, -1, 2, 1);break;
			case -4:add_triple(if_not_op, (int)const_string, triple_list_index + 2, -1, 3, 1);break;
			default: add_triple(if_not_op, exp, triple_list_index + 2, -1, 1, 1);
		}
	...
	if(childNum == 7)
	{
		...
		switch(exp){
			...
			case -3:add_triple(if_not_op, const_value, triple_list_index + 2, -1, 2, 1);break;
			case -4:add_triple(if_not_op, (int)const_string, triple_list_index + 2, -1, 3, 1);break;
			default: add_triple(if_not_op, (int)exp, triple_list_index + 2, -1, 1, 1);
		}
		backpatch = triple_list_index-1;
...
int for_code(AST_NODE *p)
{
	...
	last = triple_list_index-1;     
	triple_list[backpatch].arg2 = (union arg)(last+2);
	add_triple(goto_op, begin, -1, -1, 1, -1);
	return triple_list_index -1;
}
					...
					if(p->leftChild->rightSibling->nodeType != DOUBLE_OP_T && p->leftChild->nodeType != DOUBLE_OP_T){
							...
							case ADDRESS_SIGN:
								{
									/*	if(op == address_op){
										...
										if(temp_symtbl->type == CHAR_T&& temp_symtbl->star_num ==0)     add_triple(op,(int)temp_ID, -1, 2, 0,-1);
										else if(temp_symtbl->type == INT_T && temp_symtbl->star_num == 0)		add_triple(op,(int)temp_ID, -1, 3, 0, -1);
										}
										else{
										add_triple(op, temp_rvalue, -1, triple_list[temp_rvalue].result_type + 2, 1, -1);
										}
										}*/
									temp_rvalue = gen_triple_code[ptr->nodeType-FUNC_OFFSET](ptr);
									if(temp_rvalue == -1){
										temp_symtbl = symtbl_query(ptr->symtbl, temp_ID, 0);
										assert(temp_symtbl != NULL);
										if(temp_symtbl->type == CHAR_T&& temp_symtbl->star_num ==0)     add_triple(address_op,(int)temp_ID, -1, 2, 0,-1);
										else if(temp_symtbl->type == INT_T && temp_symtbl->star_num == 0)		add_triple(address_op,(int)temp_ID, -1, 3, 0, -1);
										return triple_list_index - 1;
									}
									else{ //& 后面是 *rvalue
										return temp_rvalue;                     
						...
						if(temp_rvalue == -1){
							...
							if(temp_symtbl->type == CHAR_T&& temp_symtbl->star_num ==0){
								...
								if(p->leftChild->content.i_content == PLUSPLUS) add_triple(add_op, (int)ptr->leftChild->content.s_content, 1, 2, 0, 2);
								else add_triple(minus_op, (int)ptr->leftChild->content.s_content, 1, 2, 0, 2);
								temp_index = triple_list_index-1;
								...
								if(p->leftChild->content.i_content == PLUSPLUS) add_triple(add_op, (int)ptr->leftChild->content.s_content, 4, 3, 0, 2);
								else add_triple(minus_op, (int)ptr->leftChild->content.s_content, 4, 3, 0, 2);
								temp_index = triple_list_index-1;
						...
						if(temp_rvalue == -1){
							...
							if(temp_symtbl->type == CHAR_T && temp_symtbl -> star_num == 0) temp_item.item_size = 0;
							else if(temp_symtbl->type == CHAR_T && temp_symtbl->star_num == 1) temp_item.item_size = 2;
							else if(temp_symtbl->type == INT_T && temp_symtbl->star_num == 1) temp_item.item_size = 3;
							else temp_item.item_size = 1;
							push(temp_item);
							return temp_rvalue;
						}
						else{
...
int return_code(AST_NODE *p)
{
	...
	switch(temp_rvalue){
		case -1:{
					...
					assert(temp_symtbl != NULL);
					if(temp_symtbl->type == CHAR_T && temp_symtbl->star_num ==0)    add_triple(return_op,(int)temp_ID, -1, -1, 0,-1);
...
void add_triple_single_op(int temp_rvalue, enum operator op, AST_NODE *ptr)
{
	...
	switch(temp_rvalue){
			case -1:{
						...
						else if(temp_symtbl->type == INT_T && temp_symtbl->star_num == 1)		add_triple(op,(int)temp_ID, -1, 3, 0, -1);
			...
			case -4:{
						add_triple(op, (int)const_string, -1, 2, 3, -1);
						break;
					}
			default:{
...
void add_triple_double_op(int temp_rvalue1, int temp_rvalue2, enum operator op, AST_NODE *ptr1, AST_NODE *ptr2)//1, 2分别对应两个操作数
{
	...
	switch(temp_rvalue1){
		case -1:{
					...
					else if(temp_symtbl->type == INT_T && temp_symtbl->star_num == 1) size_type1 = 3;
					else size_type1 = 1;
					var_type1 = 0;
					break;
				}
	...
	switch(temp_rvalue2){
		case -1:{
					...
					else if(temp_symtbl->type == INT_T && temp_symtbl->star_num == 1) size_type2 = 3;
					else size_type2 = 1;
					var_type2 = 0;
					break;
				}
	...
	if(op == add_op || op == minus_op || op == multiply_op || op == assign_op || op == star_assign_op){
		switch(op){
			case add_op:{
							if(size_type1 == 1 || size_type1 == 2 || size_type1 == 3){
								if(size_type2 == 0){
									add_triple(char_to_int_op, var2, -1, 1, var_type2, -1);
									temp_index = triple_list_index - 1;
									if(size_type1 == 1 || size_type1 == 2){
										add_triple(add_op, var1, temp_index, size_type1,var_type1, 1);								
									}
									else add_triple(adds_op, var1, temp_index, size_type1 ,var_type1, 1);
								}
								else{
									if(size_type1 == 1 || size_type1 == 2){
										add_triple(add_op, var1, var2, size_type1,var_type1, var_type2);								
									}
									else add_triple(adds_op, var1, var2, size_type1 ,var_type1, var_type2);
								}
							}
							else if(size_type2 == 1 || size_type2 == 2 || size_type2 == 3){
								if(size_type1 == 0){
									add_triple(char_to_int_op, var1, -1, 1, var_type1, -1);	
									temp_index = triple_list_index - 1;
									if(size_type2 == 1 || size_type2 == 2){
										add_triple(add_op, var2, temp_index, size_type2,var_type2, 1);
									}
									else add_triple(adds_op, var2, temp_index, size_type2 ,var_type2, 1);								
								}
								else {
									if(size_type2 == 1 || size_type2 == 2){
										add_triple(add_op, var1, var2, size_type2,var_type1, var_type2);
									}
									else add_triple(adds_op, var1, var2, size_type2 ,var_type1, var_type2);								
								}
							}
							else add_triple(adds_op, var1, var2,size_type1, var_type1, var_type2);
							break;
			...
			case minus_op:{
							  if(size_type1 == 1 || size_type1 == 2 || size_type1 == 3){
								  if(size_type2 == 0){
									  ...
									  temp_index = triple_list_index - 1;
									  if(size_type1 == 1 || size_type1 == 2){
										  add_triple(minus_op, var1, temp_index, size_type1,var_type1, 1);								
									  }
									  else add_triple(minuss_op, var1, temp_index, size_type1 ,var_type1, 1);
								  }
								  else if(size_type2 == 1){
									  if(size_type1 == 1 || size_type1 == 2){
										  add_triple(minus_op, var1, var2, size_type1,var_type1, var_type2);								
									  }
									  else add_triple(minuss_op, var1, var2, size_type1 ,var_type1, var_type2);
								  }
								  else if(size_type2 == 2){
									  add_triple(minus_op,var1,var2,size_type1, var_type1, var_type2);
								  }
								  else {
									  add_triple(minus_op,var1,var2,size_type1,var_type1, var_type2);
									  temp_index = triple_list_index - 1;
									  add_triple(r_shift, temp_index, -1, 1, 1, -1);
								  }
							  }
							  else if(size_type2 == 1 || size_type2 == 2 || size_type2 == 3){
								  if(size_type1 == 0){
								  ...
								  else if(size_type1 == 1){
									  if(size_type1 == 1 || size_type1 == 2){
										  add_triple(minus_op, var1, var2, size_type1,var_type1, var_type2);								
									  }
									  else add_triple(minuss_op, var1, var2, size_type1 ,var_type1, var_type2);
								  }
								  else if(size_type1 == 2){
									  add_triple(minus_op,var1,var2,size_type1, var_type1, var_type2);
								  }
								  else {
									  add_triple(minus_op,var1,var2,size_type1,var_type1, var_type2);
							  ...
							  else add_triple(minus_op, var1, var2, size_type1, var_type1, var_type2);
							  break;
						  }
			case multiply_op:{
								 if(size_type1 != 0 && size_type2 == 0){
									 add_triple(char_to_int_op, var2, -1, 1, var_type2, -1);
									 temp_index = triple_list_index -1;
									 add_triple(multiply_op, var1, temp_index, 1, var_type1, 1);
								 }
								 else if(size_type1 == 0 && size_type2 != 0){
									 add_triple(char_to_int_op, var1, -1, 1, var_type1, -1);
									 temp_index = triple_list_index -1;
									 add_triple(multiply_op, temp_index, var2, 1, 1,var_type2);
								 }
								 else add_triple(multiply_op, var1, var2,size_type1, var_type1, var_type2);
								 break;
							 }
			case assign_op:{
							   if(size_type1 == 0 && size_type2 !=0){
								   ...
								   temp_index = triple_list_index - 1;
								   add_triple(assign_op, var1, temp_index, 0, var_type1, 1);
							   }
							   else if(size_type1 != 0 && size_type2 == 0){
								   add_triple(char_to_int_op, var2, -1, 1, var_type2, -1);
								   temp_index = triple_list_index - 1;
								   add_triple(assign_op, var1, temp_index, 1, var_type1, 1);					   
							   }
							   else add_triple(assign_op, var1, var2, size_type1, var_type1, var_type2);
							   break;
						   }
			case star_assign_op:{
									if(size_type1 == 3 && size_type2 == 0){
										add_triple(char_to_int_op, var2, -1, 1, var_type2, -1);
										temp_index = triple_list_index-1;
										add_triple(op,var1,temp_index,1,1,1);
									}
									else if(size_type1 == 2 && (size_type2 == 1 || size_type2 == 2 || size_type2 == 3))
									{
			...
			default:break;
					/*
					   if(op == assign_op && size_type1 == 0 && (size_type2 == 1 || size_type2 == 2 || size_type2 == 3)){
					   add_triple(int_to_char_op, var2, -1, 0, var_type2, -1);
					   temp_index = triple_list_index-1;
					   ...
					   else if((size_type1 == 1 || size_type1 == 2 || size_type ==3) && size_type2 == 0){
					//Brills modified here: -1 for var_type2??
					add_triple(char_to_int_op, var2,-1,1,var_type2,-1);
					temp_index = triple_list_index-1;

					add_triple(op, var1, temp_index, size_type1, var_type1, 1);      
					}
